...
function updateAnalysis() {
    ...
    console.log('📊 Updating Analysis page...');
    
    // Update overview cards
    updateAnalysisOverview();
    
    // Update all charts
    updateShiftComparisonChart();
    updateHourlyProductivityChart();
    updateProcessTimeChart();
    updateProcessQualityChart();
    updateWorkerDistributionChart();
    updateOverlapWorkersChart();
...
function updateAnalysisOverview() {
    ...
    const avgEfficiency = validData.length > 0 
        ? ((validData.reduce((sum, r) => sum + r.workerActMins, 0) / (validData.length * 660)) * 100).toFixed(1)
        : 0;
    
    // Calculate quality rate
    const qualityRate = data.length > 0 
        ? ((validData.length / data.length) * 100).toFixed(1)
        : 0;
    
    // Find bottleneck process (process with most invalid records)
    const processInvalid = {};
    data.filter(r => r.validFlag === 0).forEach(r => {
        const proc = r.foDesc3 || r.foDesc2 || r.foDesc || 'Unknown';
        processInvalid[proc] = (processInvalid[proc] || 0) + 1;
    });
    const bottleneck = Object.entries(processInvalid).length > 0
        ? Object.entries(processInvalid).sort((a, b) => b[1] - a[1])[0][0]
        : '-';
    
    // Update DOM
    document.getElementById('analysisOverlapTime').textContent = `${overlapRemoved.toLocaleString()} min`;
    document.getElementById('analysisAvgEfficiency').textContent = `${avgEfficiency}%`;
    document.getElementById('analysisQualityRate').textContent = `${qualityRate}%`;
    document.getElementById('analysisBottleneck').textContent = bottleneck.substring(0, 20) + (bottleneck.length > 20 ? '...' : '');
}

// Shift Comparison Chart
function updateShiftComparisonChart() {
    const data = AppState.processedData;
    const shiftData = {};
    
    data.forEach(r => {
        const shift = r.workingShift || 'Unknown';
        if (!shiftData[shift]) {
            shiftData[shift] = { total: 0, valid: 0, minutes: 0 };
        }
        shiftData[shift].total++;
        if (r.validFlag === 1) {
            shiftData[shift].valid++;
            shiftData[shift].minutes += r.workerActMins || 0;
        }
    });
    
    const labels = Object.keys(shiftData);
    const qualityRates = labels.map(shift => 
        (shiftData[shift].valid / shiftData[shift].total * 100).toFixed(1)
    );
    const avgMinutes = labels.map(shift => 
...
function updateHourlyProductivityChart() {
    const data = AppState.processedData.filter(r => r.validFlag === 1);
    const hourlyData = Array(24).fill(0);
    
    data.forEach(r => {
        const hour = new Date(r.startDatetime).getHours();
        hourlyData[hour] += r.workerActMins || 0;
    });
...
function updateProcessTimeChart() {
    ...
    data.forEach(r => {
        const proc = r.foDesc3 || r.foDesc2 || r.foDesc || 'Unknown';
        if (!processData[proc]) {
            processData[proc] = { total: 0, count: 0 };
        }
        processData[proc].total += r.workerActMins || 0;
        processData[proc].count++;
    });
    
    const sorted = Object.entries(processData)
    ...
    AnalysisCharts.processTime = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Avg Minutes',
                data: sorted.map(p => p.avg.toFixed(0)),
...
// Process Quality Rate Chart
function updateProcessQualityChart() {
    const data = AppState.processedData;
    const processData = {};
    ...
    data.forEach(r => {
        const proc = r.foDesc3 || r.foDesc2 || r.foDesc || 'Unknown';
        if (!processData[proc]) {
            processData[proc] = { total: 0, valid: 0 };
        }
        processData[proc].total++;
        if (r.validFlag === 1) processData[proc].valid++;
    });
    ...
    const sorted = Object.entries(processData)
        ...
        .filter(p => p.count >= 5) // At least 5 records
    ...
    AnalysisCharts.processQuality = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Quality Rate (%)',
                data: sorted.map(p => p.rate.toFixed(1)),
                backgroundColor: sorted.map(p => 
                    p.rate >= 80 ? 'rgba(16, 185, 129, 0.7)' :
                    p.rate >= 50 ? 'rgba(251, 191, 36, 0.7)' :
                    'rgba(239, 68, 68, 0.7)'
                )
            }]
        },
...
// Worker Efficiency Distribution Chart
function updateWorkerDistributionChart() {
    const workerAgg = aggregateByWorker(AppState.processedData);
    const workerSummary = aggregateByWorkerOnly(workerAgg);
    ...
    workerSummary.forEach(w => {
        if (w.workRate >= 80) distribution['Excellent (≥80%)']++;
        else if (w.workRate >= 50) distribution['Normal (50-80%)']++;
        else if (w.workRate >= 30) distribution['Poor (30-50%)']++;
...
// Top Workers with Time Overlap
function updateOverlapWorkersChart() {
    const data = AppState.processedData;
    const workerOverlap = {};
    
    data.forEach(r => {
        const worker = r.workerName;
        const original = (new Date(r.endDatetime) - new Date(r.startDatetime)) / 60000;
        const adjusted = r.workerActMins || 0;
        const overlap = original - adjusted;
        
        if (!workerOverlap[worker]) {
            workerOverlap[worker] = 0;
        }
        workerOverlap[worker] += overlap;
    });
    
    const sorted = Object.entries(workerOverlap)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 10);
    ...
    AnalysisCharts.overlapWorkers = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Overlap Removed (min)',
                data: sorted.map(([_, overlap]) => Math.round(overlap)),
        ...
        options: {
            ...
            scales: {
                x: {
                    beginAtZero: true,
                    title: { display: true, text: 'Minutes Removed' }
                }
            }
        }
    });
...
function updateDailyTrendChart() {
    ...
    data.forEach(r => {
        ...
        if (!dailyData[date]) {
            dailyData[date] = { minutes: 0, records: 0 };
        }
        dailyData[date].minutes += r.workerActMins || 0;
        dailyData[date].records++;
    });
    ...
    AnalysisCharts.dailyTrend = new Chart(ctx, {
        type: 'line',
        data: {
            labels: sorted.map(([date, _]) => date),
            datasets: [{
                label: 'Total Minutes',
                data: sorted.map(([_, data]) => data.minutes),
                borderColor: 'rgba(59, 130, 246, 1)',
                backgroundColor: 'rgba(59, 130, 246, 0.1)',
                tension: 0.4,
                fill: true,
                ...
                label: 'Record Count',
                data: sorted.map(([_, data]) => data.records),
...
function updateInsightsTable() {
    const insights = [];
    const data = AppState.processedData;
    const workerAgg = aggregateByWorker(data);
    const workerSummary = aggregateByWorkerOnly(workerAgg);
    
    // Insight 1: Quality rate
    const validRate = (data.filter(r => r.validFlag === 1).length / data.length * 100).toFixed(1);
    if (validRate < 70) {
        insights.push({
            type: 'warning',
            icon: 'fa-exclamation-triangle',
            title: 'Low Quality Rate',
            description: `Overall quality rate is ${validRate}%. Consider investigating processes with high invalid rates.`
        });
        ...
        insights.push({
            ...
            description: `Outstanding quality rate of ${validRate}%. Keep up the good work!`
        });
    }
    
    // Insight 2: Worker performance
    const criticalWorkers = workerSummary.filter(w => w.workRate < 30).length;
    if (criticalWorkers > 0) {
        insights.push({
            type: 'warning',
            icon: 'fa-users',
            title: 'Critical Workers Detected',
            description: `${criticalWorkers} worker(s) with efficiency below 30%. Provide training or support.`
        });
    }
    ...
    if (overlapPercent > 20) {
        insights.push({
            type: 'info',
            icon: 'fa-clock',
            title: 'Significant Time Overlap',
            description: `${overlapPercent}% of recorded time was overlap. Review work assignment to reduce multi-tasking.`
        });
    }
    
    // Insight 4: Best shift
    const shiftData = {};
    data.forEach(r => {
        const shift = r.workingShift || 'Unknown';
        if (!shiftData[shift]) shiftData[shift] = { total: 0, valid: 0 };
        shiftData[shift].total++;
        if (r.validFlag === 1) shiftData[shift].valid++;
    });
    
    const bestShift = Object.entries(shiftData)
        .map(([shift, data]) => ({ shift, rate: data.valid / data.total }))
    ...
    if (bestShift) {
        insights.push({
            ...
            description: `${bestShift.shift} shift has the highest quality rate at ${(bestShift.rate * 100).toFixed(1)}%.`
        });
    }
    
    // Render insights
    const container = document.getElementById('insightsTable');
    container.innerHTML = insights.map(insight => `
        <div class="flex items-start p-4 rounded-lg ${
            ...
            <i class="fas ${insight.icon} text-2xl mr-4 ${
                insight.type === 'success' ? 'text-green-600' :
                insight.type === 'warning' ? 'text-yellow-600' :
                'text-blue-600'
            }"></i>
            <div>
                <h4 class="font-bold text-gray-800 mb-1">${insight.title}</h4>
