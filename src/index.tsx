...
// API: 엑셀 데이터 저장 (백그라운드)
// 청크 업로드 API - 프론트엔드에서 데이터를 나눠서 보냄
app.post('/api/upload-chunk', async (c) => {
  try {
    const { env } = c
    const body = await c.req.json()
    
    const { uploadId, chunk, chunkIndex, totalChunks, isFirst, isLast, metadata } = body
    
    console.log(`📦 Chunk upload: ${chunkIndex + 1}/${totalChunks}, uploadId=${uploadId}, records=${chunk?.length || 0}`)
    
    // 첫 번째 청크: 업로드 레코드 생성
    let finalUploadId = uploadId
    if (isFirst && metadata) {
      const { filename, fileSize, totalRecords, uniqueWorkers, dateRangeStart, dateRangeEnd } = metadata
      ...
      finalUploadId = uploadResult.meta.last_row_id as number
      console.log(`✅ Created upload record #${finalUploadId}`)
    }
    ...
    if (chunk && chunk.length > 0) {
      ...
      // 진행 상황 업데이트
      const currentProgress = (chunkIndex + 1) * chunk.length
      await env.DB.prepare(`
        UPDATE excel_uploads 
        SET progress_current = ?
        WHERE id = ?
      `).bind(currentProgress, finalUploadId).run()
      
      console.log(`✅ Chunk ${chunkIndex + 1}/${totalChunks} saved (${chunk.length} records)`)
    }
    
    // 마지막 청크: 메타데이터 저장 및 완료 처리
    if (isLast && metadata) {
      const { processMapping, shiftCalendar } = metadata
      ...
      await env.DB.prepare(`
        ...
        WHERE id = ?
      `).bind(finalUploadId).run()
      
      console.log(`✅ Upload #${finalUploadId} completed!`)
    }
...
// 기존 업로드 API (레거시, 백그라운드 처리 - 큰 데이터셋에서는 실패 가능)
app.post('/api/upload', async (c) => {
  try {
    ...
    console.log('⚠️ WARNING: Using legacy upload API. For large datasets, use /api/upload-chunk instead.')
