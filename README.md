...
**✨ 새로운 기능 (v3.0.2)**: 
- 🐛 **Outlier 필터링 완전 수정**: aggregation pipeline 전체에서 outlier가 제거되도록 보장
- 📊 **데이터 일관성 보장**: workerAgg → workerSummary 전달 시 이미 필터된 데이터 사용
- 🔍 **성능 개선**: 불필요한 데이터 재처리 제거, 효율적인 집계 프로세스 
...
### v3.0.2 (2026-02-21) ✨ **Critical Fix**
- 🐛 **Outlier 필터링 완전 수정**: 
  - 근본 원인: `aggregateByWorkerOnly()` 함수가 원본 데이터를 재처리하여 outlier 필터를 우회
  - 해결: `aggregateByWorkerOnly()`를 수정하여 이미 필터된 `workerAgg` 데이터만 사용
  - 결과: `aggregateByWorker()`에서 제거된 outlier가 차트와 밴드에 다시 나타나지 않음
  - 성능 개선: 불필요한 데이터 재처리 제거, 메모리 사용량 감소
- 📊 **데이터 일관성 보장**: 
  - updateReport() → aggregateByWorker() → aggregateByWorkerOnly() pipeline 전체에서 일관성 보장
  - KPIs, Performance Bands, Charts 모두 동일한 필터된 데이터 사용
  - 필터 적용 순서: UI 필터 → Rework 제외 → S/T > 0 검증 → Outlier 제거

### v3.0.1 (2026-02-21)
- ✅ **Outlier Threshold 필터 Mode 분리**:
  - **Work Efficiency 모드**: Outlier Threshold UI 표시, 필터링 활성화
  - **Time Utilization 모드**: Outlier Threshold UI 숨김, 필터링 비활성화
- ✅ **필터링 로직 개선**: W/O 레벨에서만 Outlier 필터링 적용 (Worker Total 레벨 제거)
- ✅ **로그 개선**: 필터링 통계 콘솔 로그 추가

### v3.0.0 (2026-02-20)
- ✅ **두 가지 성과 지표 시스템 추가**:
  - **Time Utilization Rate**: 실제 작업 시간 / 가용 시간 × 100%
  - **Work Efficiency Rate**: 할당 표준 시간 / 실제 작업 시간 × 100%
- ✅ **지표 토글 버튼**: 실시간 지표 전환 (시계 아이콘 ⇄ 번개 아이콘)
- ✅ **작업자 상세 모달 분기 로직**: 
  - Time Utilization: 일별/시간별 작업 시간, 중복 제거 표시
  - Work Efficiency: S/T, Rate, Assigned, Actual, Efficiency 상세 표시
- ✅ **Rework 자동 필터링**: W column = true인 레코드 제외
- ✅ **Worker S/T > 0 조건 적용**: Work Efficiency 모드에서만 S/T > 0 레코드 사용
- ✅ **성과 밴드 재설계**: Excellent (>100%), Good (80-100%), Normal (60-80%), Poor (40-60%), Critical (<40%)

---

**제작자**: twokomi  
**마지막 업데이트**: 2026-02-21 (v3.0.2)
