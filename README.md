...
### v3.0.1 (2026-02-21) 🔧 Bug Fix
- ✅ **Outlier Threshold 필터링 수정**:
  - Outlier Threshold가 Efficiency 모드일 때만 적용되도록 수정
  - Utilization 모드에서는 Outlier Threshold UI 숨김 처리
  - W/O 레벨에서만 필터링 적용 (Worker Total 레벨은 이미 필터링된 데이터의 평균이므로 불필요)
- ✅ **D1 데이터베이스 에러 핸들링 개선**:
  - 로컬 개발 환경에서 D1 바인딩이 없을 때 graceful fallback 적용
  - /api/uploads 엔드포인트 에러 처리 강화
- ✅ **Working Day/Shift 계산 로직 검증 완료**:
  - Day shift: 06:00-17:59 (같은 날짜)
  - Night shift: 18:00-05:59 (00:00-05:59는 전날)
  - actualShift는 ShiftCalendar에서 정확히 매핑됨
